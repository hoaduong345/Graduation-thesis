...
const ProductController = {
  ...
  addImagesByCategoryID : async(req, res) =>{
    try {
      const { url, idcategory } = req.body;
      ...
      const data = await prisma.categoryImage.create({
        data: newImages,
      });
      res.status(200).json("Thêm hình thành công");
    } catch (error) {
      console.error(error);
      res.status(500).json(error.message);
    }
  ...
  deleteImagesByCategoryID : async(req, res) =>{
    const categoryID = parseInt(req.params.id);
    try {
      const imagesToDelete = await prisma.categoryImage.findMany({
        where: {
          id: categoryID,
        },
      });
  ...
  updateImagesByCategoryID : async(req, res) =>{
    try {
      const { id } = req.params;
      const { url, idcategory } = req.body;
